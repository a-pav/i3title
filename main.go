...
func readTitle() {
	...
	time.Sleep(time.Duration(cn.TiMod.Delay) * time.Second)
...
func readLine() {
	...
	scanner.Buffer(make([]byte, 0, cn.BufSize), 0)
...
func makeTitle(title string) string {
	title = cn.Replacer.Replace(title)
...
func printline() {
	fmt.Fprintf(os.Stdout, "%s\n",
		bytes.Replace(LINE, []byte(cn.TiMod.PH), []byte(TITLE), 1),
	)
}
